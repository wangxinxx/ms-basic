    ...
    <@ms.panel>
    	<@ms.form name="dictForm" isvalidation=true>
    		<@ms.hidden name="dictId" value="${dictEntity.dictId?default('')}"/>
    			...
    			<@ms.text label="备注信息" name="dictRemarks" value="${dictEntity.dictRemarks?default('')}"  width="240px;" placeholder="请输入备注信息" validation={"required":"true","maxlength":"50","data-bv-stringlength-message":"备注信息长度不能超过五十个字符长度!", "data-bv-notempty-message":"必填项目"}/>
    	</@ms.form>
    </@ms.panel>
</@ms.html5>
<script>
	var url = "${managerPath}/basic/dict/save.do";
	if($("input[name = 'dictId']").val() > 0){
		url = "${managerPath}/basic/dict/update.do";
		$(".btn-success").text("更新");
	}
	//编辑按钮onclick
	function save() {
		...
		$.ajax({
			...
			success: function(status) {
				if(status.result == true) { 
					...
					location.href = "${managerPath}/basic/dict/index.do";
				}
				else{
